...
ObjSonic_MakeWaterfallSplash:
	...
	move.b	stage_frames+1,d0		; Are we on an odd numbered frame?
	andi.b	#1,d0
	bne.s	.End				; If so, branch
...
.PosInertia:
	...
	cmpi.w	#$1B00,obj.x(a0)			; Are we on the 3D ramp at the start of the level?
...
.CtrlLock:
	...
	clr.w	time_warp_timer			; Disable time warping
	clr.b	time_warp
	bra.s	.SkipControl
...
.SkipControl:
	...
	tst.b	wind_tunnel_flag		; Are we in a wind tunnel?
	beq.s	.NoWindTunnel			; If not, branch
	tst.b	obj.anim_id(a0)			; Are we in the walking animation?
	bne.s	.NoWindTunnel			; If not, branch
	move.b	obj.prev_anim_id(a0),obj.anim_id(a0)		; Set animation to the previously saved animation ID
...
ObjSonic_RecordPos:
	...
	addq.b	#4,player_positions_index+1.w	; Advance position buffer index
	rts
...
.HasWater:
	...
	cmpi.w	#$C8,obj.x(a0)			; Are we in the wrapping section?
	bcs.s	.End				; If so, branch

.NotAct2:
	move.w	water_height,d0			; Are we in the water?
	cmp.w	obj.y(a0),d0
	bge.s	.OutWater			; If not, branch

	bset	#6,obj.flags(a0)		; Mark as underwater
	...
	move.b	#$21,bubbles_object		; Create bubbles that come out of our mouth
	move.b	#$81,bubbles_object+obj.subtype

	move.w	#$300,player_top_speed		; Set to water physics
	move.w	#6,player_acceleration
	move.w	#$40,player_deceleration

	asr	obj.x_speed(a0)			; Slow ourselves down in the water
	asr	obj.y_speed(a0)
	asr	obj.y_speed(a0)
...
.OutWater:
	tst.w	obj.y_speed(a0)			; Are we moving vertically?
	beq.s	.LeaveWater			; If not, branch
	bpl.s	.End				; If we are moving downwards, branch

.LeaveWater:
	bclr	#6,obj.flags(a0)			; Mark as not underwater
	...
	asl	obj.y_speed(a0)			; Accelerate ourselves out of the water
	beq.w	.End				; If we are still moving up too slowly, branch
	cmpi.w	#-$1000,obj.y_speed(a0)		; Are we moving up too fast?
	bgt.s	.LoadSplash			; If not, branch
	move.w	#-$1000,obj.y_speed(a0)		; Cap our speed
...
SaveTimeWarpData:
	...
	bclr	#6,warp_player_flags		; Not underwater
	move.b	water_routine,warp_water_routine
	move.w	bottom_bound,warp_bottom_bound
	move.w	camera_fg_x.w,warp_camera_fg_x
	move.w	camera_fg_y.w,warp_camera_fg_y
...
.CapTime:
	...
	move.b	mini_player,warp_mini_player
	rts
...
ObjSonic_TimeWarp:
	cmpi.w	#0,zone_act			; Are we in Palmtree Panic act 1?
...
.Past:
	cmpi.w	#$900,obj.x(a0)			; Are we in the first 3D ramp section?
...
ObjSonic_MdAir:
	...
	cmpi.b	#$15,obj.anim_id(a0)			; Were we breathing in a bubble?
...
.DoRoll:
	...
	tst.b	mini_player			; Are we miniature?
	beq.s	.NotMini			; If not, branch
	move.b	#8,obj.collide_height(a0)			; Set miniature rolling hitbox size
	move.b	#5,obj.collide_width(a0)
	addq.w	#2,obj.y(a0)
...
.GotAngle:
	...
	move.b	#8,obj.collide_height(a0)			; Set miniature jumping hitbox size
	move.b	#5,obj.collide_width(a0)
	addq.w	#2,obj.y(a0)
...
.NotJump:
	cmpi.w	#-$FC0,obj.y_speed(a0)		; Is our Y velocity less than -15.75?
	bge.s	.End2				; If not, branch
	move.w	#-$FC0,obj.y_speed(a0)		; Cap our Y velocity at -15.75
