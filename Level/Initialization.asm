...
Start:
	...
	cmpi.l	#'init',init_flag		; Have we already initialized?
	beq.w	.GameInit			; If so, branch

.DoInit:
	lea	unknown_buffer,a6		; Clear unknown buffer
	moveq	#0,d7
	move.w	#$400/4-1,d6
	move.w	#$200/4-1,d6
...
.ClearRAM:
	...
	andi.b	#$C0,d0
	move.b	d0,hardware_version

	move.l	#'init',init_flag		; Mark as done

.GameInit:
	bsr.w	InitVDP				; Initialize VDP
	bsr.w	InitControllers			; Initialize joypads

	move.b	#0,game_mode			; Set game mode to "level"

	move.b	game_mode,d0			; Go to the current game mode routine
	andi.w	#$1C,d0
