...
ResetSavedObjFlags:
	lea	map_object_states,a2		; Reset current entry IDs
	move.w	#$101,(a2)+

	move.w	#(MAP_OBJECT_STATE_SLOTS*3)/4-1,d0
...
SpawnObjects_Init:
	addq.b	#2,map_object_spawn		; Next routine

	lea	ObjectLayouts,a0		; Get object layout
	movea.l	a0,a1
	adda.w	(a0),a0
	move.l	a0,map_object_chunk_right
	move.l	a0,map_object_chunk_left
	adda.w	2(a1),a1			; Get null object layout
	move.l	a1,map_object_chunk_null_1
	move.l	a1,map_object_chunk_null_2

	lea	map_object_states,a2		; Reset current saved object flag entry IDs
	move.w	#$101,(a2)

	moveq	#0,d2				; Get initial right chunk position
	move.w	camera_fg_x,d6			; (max(0, camera X - $80) & $FF80)
	subi.w	#$80,d6
...
.GotLeftBound:
	...
	movea.l	map_object_chunk_right,a0	; Find right chunk
...
.FoundRightChunk:
	move.l	a0,map_object_chunk_right	; Set right chunk address

	movea.l	map_object_chunk_left,a0	; Find left chunk
...
.FoundLeftChunk:
	move.l	a0,map_object_chunk_left	; Set left chunk address

	move.w	#-1,prev_map_object_chunk	; Force object spawns
...
SpawnObjects_Main:
	lea	map_object_states,a2		; Get saved object flags

	moveq	#0,d2				; Get current chunk position
	move.w	camera_fg_x,d6
	andi.w	#$FF80,d6
	cmp.w	prev_map_object_chunk,d6	; Has it changed?
	...
	nop
	nop
	move.w	d6,prev_map_object_chunk	; Update chunk position

; ------------------------------------------------------------------------------

SpawnObjects_Backward:
	movea.l	map_object_chunk_left,a0	; Get new left chunk position
...
.SpawnDone:
	...
	movea.l	map_object_chunk_right,a0	; Get new right chunk position
...
.SpawnDone:
	...
	movea.l	map_object_chunk_left,a0	; Get left chunk position
...
.Spawn:
	...
	move.w	(a0)+,obj.x(a1)			; Set X position
	...
	move.b	d2,obj.state_id(a1)		; Set saved flags entry ID

.NoSavedFlags:
	move.b	d0,obj.id(a1)			; Set object ID
...
.SetSubtypes:
	move.b	(a0)+,obj.subtype(a1)		; Set subtype
	move.b	(a0)+,d0			; Skip time zone settings
	move.b	(a0)+,obj.subtype_2(a1)		; Set secondary subtype
...
FindObjSlot:
	lea	object_spawn_pool,a1		; Object pool
	move.w	#OBJECT_SPAWN_COUNT-1,d0	; Number of slots to check
...
FindNextObjSlot:
	...
	move.w	#objects_end,d0			; Get number of slots to check
	sub.w	a0,d0
	...
	subq.w	#1+1,d0				; Decrement for loop
...
CheckObjDespawn:
	move.w	obj.x(a0),d0			; Get our X position
...
DespawnObject:
	...
	lea	map_object_states,a1		; Saved object flags table
