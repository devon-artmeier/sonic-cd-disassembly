; ------------------------------------------------------------------------------
; Sonic CD (1993) Disassembly
; By Devon Artmeier
; ------------------------------------------------------------------------------
; "Press Start" objects (special stage)
; ------------------------------------------------------------------------------

ObjPressStart:
	moveq	#0,d0					; Run routine
	move.b	obj.routine(a0),d0
	add.w	d0,d0
	move.w	.Index(pc,d0.w),d0
	jsr	.Index(pc,d0.w)

	bsr.w	DrawObject				; Draw sprite
	rts

; ------------------------------------------------------------------------------

.Index:
	dc.w	ObjPressStart_Init-.Index
	dc.w	ObjPressStart_Main-.Index

; ------------------------------------------------------------------------------

ObjPressStart_Init:
	move.w	#$856A,obj.sprite_tile(a0)		; Base tile ID
	move.l	#MapSpr_PressStart,obj.sprites(a0)	; Mappings
	move.w	#84+128,obj.sprite_x(a0)		; Set position
	move.w	#80+128,obj.sprite_y(a0)
	moveq	#0,d0					; Set animation
	bsr.w	SetObjAnim
	addq.b	#1,obj.routine(a0)			; Set routine to main

; ------------------------------------------------------------------------------

ObjPressStart_Main:
	addq.b	#1,obj.timer(a0)			; Increment timer
	bset	#2,obj.flags(a0)			; Flash every 16 frames
	btst	#4,obj.timer(a0)
	bne.s	.End
	bclr	#2,obj.flags(a0)

.End:
	rts

; ------------------------------------------------------------------------------

MapSpr_PressStart:
	include	"Special Stage/Objects/Press Start/Data/Mappings.asm"
	even

; ------------------------------------------------------------------------------
